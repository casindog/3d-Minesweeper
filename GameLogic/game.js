...
class Game {
    ...
    gameSetup() {
        ...
        let arr = new Array(this.layers+2);
        for (let i=0; i<this.layers; i++) {
            arr[i] = new Array(this.layers+2).fill(null);
        }

        for (let i=0; i<this.layers+2; i++) {
            if (i === 0 || i ===this.layers+1) {
                this.planes.push(arr);
            } else {
...
// Actually, this is going to be very important. 
let g = new Game()

g.planes.forEach((plane,idx) => {
    if (idx===0 || idx===g.planes.length-1) {
        console.log(plane)
        console.log('\n')
    } else {
        plane.render();
